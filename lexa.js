...
const _simi = JSON.parse(fs.readFileSync('./data/simi.json'));
...
const Pkode = 'pais'
const viKey = 'Vinzapi'
const apiSimi = 'ZStT7H-lhrWAdktifUH.lR3~i~Au3QB0ogFJ113B'
...
//-- Simsimi
case 'simi':
			if (!isGroup) return reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!')
            if (!isGroupAdmins) return reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!')
			if (args.length !== 1) return reply(from, `Untuk mengaktifkan simi-simi pada Group Chat\n\nPenggunaan\n${prefix}simi on --mengaktifkan\n${prefix}simi off --nonaktifkan\n`)
			if (args[0] == 'on') {
				_simi.push(from)
				fs.writeFileSync('./data/simi.json', JSON.stringify(from))
                reply(from, 'Mengaktifkan bot simi-simi!')
			} else if (args[0] == 'off') {
				let inxx = _simi.indexOf(from)
				simi.splice(inxx, 1)
				fs.writeFileSync('./data/simi.json', JSON.stringify(from))
				reply(from, 'Menonaktifkan bot simi-simi!')
			} else {
				reply(from, `Untuk mengaktifkan simi-simi pada Group Chat\n\nPenggunaan\n${prefix}simi on --mengaktifkan\n${prefix}simi off --nonaktifkan\n`)
			}
			break
                		case 'simisimi':
			if (!isGroupMsg) return reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!')
			reply(from, `Untuk mengaktifkan simi-simi pada Group Chat\n\nPenggunaan\n${prefix}simi on --mengaktifkan\n${prefix}simi off --nonaktifkan\n`)
			break

 if ((!isCmd && isGroup && chatId && isSimi) && message.type === 'chat') {
                axios.get(`https://videfikri.com/api/simsimi/?teks=${encodeURIComponent(message.body)}`)
                .then((res) => {
                    if (res.data.result.status == 404) return sendText(ownerNumber, `${res.data.result.status}\n\n${res.data.result.pesan}`)
                    reply(from, `Simi berkata: ${res.data.result.jawaban}`, id)
                })
                .catch((err) => {
                    reply(from, `${err}`)
                })
...
case 'cie':
    ...
    if (!isGroup) return reply(group())
					jds = []
					meh = ["Putus","Jadian","Tunangan","Menikah","mojok","Gelud","Ciuman di pinggir jalan"]
...
if (args.length < 1) return reply('Tag Target')
					...
					karn = ["Jelek","Ganteng","Miskin","Kaya","Cantik","Tukang PHO","Sering Di PHP in","Wibu","Tukang PHO","Mirip Monyet"]
					karna = karn[Math.floor(Math.random() * karn.length)]
					jad = ["Monyet","kambing","Jelek","Patung","Kodok","Babi","Anak Anjing","Sapi"]
...
//-- owner bot
case 'owner':
    nomor = '6285331291446@s.whatsapp.net'
    owner = await fs.readFileSync('./docs/mrf.jpeg').toString('base64')
    capt = "𝗗𝗲𝘃𝗲𝗹𝗼𝗽𝗲𝗿\nIG : Bay\nWhatsapp BOT : I'm Bay V.3"
    const been = {
    text: `@${nomor.split("@")[0]} , chat aja kalo ada perlu`,
  contextInfo: {
mentionedJid: [nomor]
  }
}
